	...
	protected function get_language($rows){
		...
		if(!empty($rows[0]->currency_id))
		{
			foreach ($rows as $key => $row) {
				...
				$currency = $db->query($query,['id'=>$row->language_id]);
				if(!empty($currency)){

					$currency[0]->name = $currency[0]->symbol . ' ('. $currency[0]->language .')';
					$rows[$key]->language_row = $currency[0];
				}
			}
		}
		return $rows;
