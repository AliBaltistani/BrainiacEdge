...
<?php else : ?>
  ...
  <div class="card">
    ...
    <div class="card-body">
      <h5 class="card-title">
        <?= (Auth::is_admin())? 'All Courses' :  'My Courses'; ?>
        <?php if(user_can('add_courses')){ ?>
          ...
          <tbody>
            ...
            <?php foreach ($rows as $row) : ?>
              <tr>
                ...
                <td> 
                 <?=($row->approved == '1')?'<span class="alert alert-success">Approved</span>' :'<span class="text-info">pending</span>'?>
                </td >
                <td> 
                <div class="form-check form-switch">
                          <input <?=($row->published == '1')?'checked' :''?>  onchange="status_change('published',this.value,'<?= $row->id ?>')" name="published" value="<?=($row->published == '1')?'0' :'1'?>" class="form-check-input" type="checkbox" id="publishedCheckChecked">
                          <!-- <label class="form-check-label" for="approvedCheckChecked">Approved</label> -->
                        </div>
                </td >
                <td>
                <?php if(user_can('edit_courses')){ ?>
                  <a href="<?= ROOT ?>/admin/courses/edit/<?= $row->id ?>">
                    <i class="bi bi-pencil-square"></i>
                  </a>
                  <?php } ?>

                  <?php if(user_can('delete_courses')){ ?>
          ...
          <tbody>
            ...
            <?php foreach ($rows as $row) : ?>
              <tr>
                ...
                <td> 
                      <div class="form-check form-switch">
                          <input <?=($row->approved == '1')?'checked' :''?> onchange="status_change('approved',this.value,'<?= $row->id ?>')"  name="approved" value="<?=($row->approved == '1')?'0' :'1'?>" class="form-check-input" type="checkbox" id="approvedCheckChecked">
                          <!-- <label class="form-check-label" for="approvedCheckChecked">Approved</label> -->
                        </div>
                </td >
                <td> 
...
<script>
function status_change(col,val,c_id) {

  var obj = {};
      obj.data_type = "change-status";
      obj.course_id = c_id;
      obj[col] = val;
