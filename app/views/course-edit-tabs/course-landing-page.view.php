...
<form>
	<div class="col-md-8 mx-auto">
	        ...
	        <div class="col-md-6 my-3">
		        ...
		        <div class="col-md-4 mb-4">
		        	<select name="currency_id" class="form-select">
						...
						<?php foreach($prices as $cp):
					     	array_push($prices_sym, $cp->symbol);
						endforeach;

							// $categories = get_categories_have_course();
							array_unique($prices_sym,SORT_REGULAR);
							$prices_sym = array_map("unserialize", array_unique(array_map("serialize", $prices_sym)));

						for($i=0; $i < count($prices_sym); $i++ ){
							?>
							
							<option    value=""><?=esc($prices_sym[$i])?></option>
						<?php }?>
						<?php endif;?>
		        	</select>
		        </div>
	        	<div class="col-md-8 mb-4">
	        		...
	        		<?php if(!empty($prices)):?>
		              <?php foreach($prices as $cat):?>
		                <option <?=set_select('price_id',$cat->id,$row->price_id)?> value="<?=$cat->id?>"><?=esc($cat->price)?></option>
		              <?php endforeach;?>
		            <?php endif;?>
		        	</select>
		...
		<div class="input-group mb-3">
          <span class="input-group-text" id="basic-addon1">Course Tags</span>
          <input value="<?=$row->tags?>" name="tags" placeholder="Business, Culture, Sport..." type="text" class="form-control" >
        	<small class="error error-tags w-100 text-danger"></small>
        </div>
