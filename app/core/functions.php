...
function get_categories_have_course($id = null)
{
	$category = new \Model\Category();
	if($id){
		$category->limit = 1;
		$query = "select cat.category , cat.slug from categories cat JOIN courses cs ON cs.category_id = cat.id order by id desc ";
		// $data['trending'] = 
		return $category->query($query);
	}else{
		$query = "select cat.category , cat.slug  from categories cat JOIN courses cs ON cat.id = cs.category_id order by cat.id desc ";
		// $data['trending'] = 
		return $category->query($query);
	}
}
...
function no_access(){
	return views_path('admin/no-access');
}
